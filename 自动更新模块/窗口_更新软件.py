...
class 窗口_更新软件(QDialog):
    ...
    def 安装更新(self):
        ...
        self.ui.lineEdit_jdxx.show()
        self.ui.lineEdit_jdxx.setText('更新中...')
        self.ui.pushButton_azgx.setEnabled(False)
        self.ui.pushButton_tgbb.setEnabled(False)
        ...
        if 系统_是否为mac系统():
            if self.mac下载地址 == "":
                self.ui.lineEdit_jdxx.setText("没有找到 ManOS 系统软件下载地址")
                return
        ...
        if 系统_是否为window系统():
            if self.win下载地址 == "":
                self.ui.lineEdit_jdxx.setText("没有找到 windows 系统软件下载地址")
                return
    ...
    def 下载完成(self, 下载结果, 保存地址):
        if not 下载结果:
            self.ui.lineEdit_jdxx.setText("下载更新失败")
            return
        if 系统_是否为mac系统():
            更新自己MacOS应用(
                资源压缩包=保存地址,
